...
> [!IMPORTANT]
> Mock 是默认运行模式，创建环境、迁移数据库、运行测试和启动演示都不需要真实凭据，
> 也不会产生 API 费用。Live 适配器已经实现，但只有显式切换 Live、配置加密凭据并
> 人工运行服务或 smoke 命令时才会访问高德、QWeather 或 LLM。
...
生产单镜像使用 `python -m roambot.entrypoint`：Mock 模式不读取凭据库；Live 模式从
权限受限的主密码文件读取，或仅在交互式 TTY 中隐藏提示。非交互 Live 启动缺少主密码
时固定退出 78，不会以空凭据继续运行。

## Provider 缓存与降级

- 地理编码、POI、距离、天气的 TTL 分别为 30 天、7 天、1 天、1 小时。
- 每个外部请求有 5 秒默认超时和按操作计数的请求预算，错误输出会脱敏且不自动重试。
- 苏州 POI 不可用时可回退内置演示数据；距离失败可回退直线估算；天气覆盖不足会排除
  对应候选；LLM 失败只回退模板理由，不影响评分与排序。
