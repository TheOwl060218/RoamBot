...
- `weights` 整体可省略；一旦提供，`weather,distance,fairness,popularity` 四个有限数值字段必须全部存在、每项在 0-100、总和必须为 100，额外字段拒绝。只有一个出发地时 `fairness` 必须为 0；两个或三个出发地时 `fairness` 固定为 20，其余三项合计 80。
...
- 公平性分。
- 景区热度/推荐度分。
- 用户自定义权重；可选项仅包括天气适配、距离远近、景区热度/推荐度。多人距离方差是固定产品规则，不作为可调项展示。
...
- 多人多地时，用户可调的天气/距离/热度显示比例仍默认为 `40/30/30`；提交时三项分别乘 80%，得到有效权重 `32/24/24`，另固定加入多人公平性 20%。
- 前端始终只显示一根三段比例条和两个分界按钮，三个区域为天气适配、距离远近、景区热度/推荐度；按钮按 5 个百分点移动、不可交叉但允许重合。多人公平性不显示为第四段，也不能由用户调整。
- 单人推荐先计算 `weighted = 天气分×w_weather + 距离分×w_distance + 热度分×w_popularity`。多人推荐计算 `weighted = (天气分×显示天气比例 + 距离分×显示距离比例 + 热度分×显示热度比例)×80% + 公平性分×20%`。`ANY` 模式的覆盖惩罚固定为 0，`COVER_ALL` 为 `20 × (1 - 匹配类型数 / 选择类型数)`；内部总值为 `clamp2(weighted - 覆盖惩罚)`。风景匹配不是额外权重，只用于硬过滤与覆盖惩罚。
- 指定地点评估复用同一四项加权公式但覆盖惩罚固定为 0；不执行候选列表排序。
- `clamp2(x)` 定义为先限制到 0-100，再使用 Python `round(x, 2)`。每日天气、区间天气、距离、公平性、热度和最终内部总值分别在各自函数出口舍入两位。API 与历史快照保留这些数值；WebUI 总值四舍五入为整数“出游匹配指数”，分项只显示定性描述。
...
- 权重控件内部使用总和恒为 100 的天气/距离/热度三个整数百分比，默认 `40/30/30`；单人和多人都显示同一根三段比例条。两个分界按钮以 5 个百分点为步长移动，只改变其左右相邻两段，允许某段为 0；重叠按钮必须视觉错开并保持独立键盘焦点和可访问名称。添加或删除同行人时显示比例不增加公平性第四段；指定地点评估也显示相同权重控件。
...
- 推荐结果卡片展示地点、距离、所选日期区间内的每日天气、整数“出游匹配指数”、分项定性描述和推荐理由。卡片固定提示“指数用于比较本次候选地点，不代表官方评价。”；不得显示面向用户的分项数值或“总分”措辞。
...
- 公开内容包含地点、城市、日期、天气摘要、出游匹配指数、推荐理由、同行人数和生成时间；删除主出发地址与全部同行地址正文，不做部分遮盖，不得包含用户名、用户标识、API key、会话标识或其他私人数据。
- 分享令牌只在创建时返回明文，数据库只保存令牌哈希；已撤销或不存在的链接返回统一的不可用状态。
- 分享页面明确标注内容是生成时的历史快照，不自动刷新天气或匹配指数。
...
- 性能：一次推荐请求以 10 秒内返回为目标；该目标依赖外部 provider 与部署区域网络，不作为固定 SLA。Railway EU West 公网环境曾实测一次 Live 推荐约 29 秒，README 与验证证据需保留这一限制。
...
- React 适合承载双模式表单、三段比例权重条、登录、收藏、历史和分享等交互状态；TypeScript 用于减少前后端字段不一致。
