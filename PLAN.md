...
以下映射用于补充实施完成后的可追溯性。一个提交覆盖多个紧密关联 task 时保留同一提交号；后续修复与完整验证记录见 `AGENT_LOG.md` 和 `docs/evidence/verification.md`。

| Task | 状态 | 主要提交或证据 |
| --- | --- | --- |
| 1.1 Brainstorming | 完成 | `53962d7` 至 `64edf9c`，批准设计为 `e786941` |
| 1.2 Writing Plans | 完成 | `e786941` |
| 1.3 陌生 agent 冷启动 | 完成 | `SPEC_PROCESS.md` 迭代 39–46，最终规划基线 `e786941` |
| M1.1 Python 包与健康检查 | 完成 | `ed0c315` |
| M1.2 请求、响应与领域模型 | 完成 | `23584e8` |
| M1.3 风景分类规则 | 完成 | `781fb98` |
| M1.4 天气、距离、公平性、热度与排序 | 完成 | `7940280`，补强 `08b3abe` |
| M1.5 Provider 协议与苏州 Mock | 完成 | `92c863e` |
| M1.6 推荐与指定地点评估服务 | 完成 | `6a23ec7` |
| M1.7 核心 API 与错误信封 | 完成 | `92600bb`，修复 `b7c2efb` |
| M1.8 里程碑验证 | 完成 | `6b3321b` |
| M2.1 SQLite 与 Alembic | 完成 | `f2e646f` |
| M2.2 密码、会话与 CSRF 基础 | 完成 | `f2e646f` |
| M2.3 认证服务 | 完成 | `f2e646f`、`f37a5f0` |
| M2.4 认证 API 与 CSRF | 完成 | `f37a5f0` |
| M2.5 收藏 | 完成 | `c4a150f` |
| M2.6 历史 | 完成 | `c4a150f` |
| M2.7 匿名只读分享 | 完成 | `f3b8012` |
| M2.8 Provider 缓存仓储 | 完成 | `f3b8012` |
| M2.9 加密凭据库与本机 CLI | 完成 | `d98155c` |
| M2.10 里程碑验证 | 完成 | `d98155c` 及 `AGENT_LOG.md` 2026-07-17/18 记录 |
| M3.1 Vite、测试入口与应用壳 | 完成 | `40267ea` |
| M3.2 类型化 API 客户端与认证上下文 | 完成 | `81cee65` |
| M3.3 双模式表单与权重 | 完成 | `81cee65`，后续 `816f2d3` |
| M3.4 推荐与评估结果工作区 | 完成 | `81cee65`，后续 `1729578` |
| M3.5 注册登录 UI | 完成 | `81cee65` |
| M3.6 收藏、历史与分享页面 | 完成 | `81cee65` |
| M3.7 响应式视觉检查 | 完成 | `81cee65`，最终收尾 `1729578` |
| M3.8 Playwright 用户旅程 | 完成 | `81cee65`，验证见 `docs/evidence/verification.md` |
| M3.9 一键测试脚本 | 完成 | `81cee65`，最终入口修复 `fce8f4d` |
| M4.1 非敏感配置与 Provider Factory | 完成 | `3af8297` |
| M4.2 脱敏 HTTP 与调用预算 | 完成 | `5fb822b` |
| M4.3 高德适配器 | 完成 | `1c9106b` |
| M4.4 QWeather 适配器 | 完成 | `2db94c7` |
| M4.5 新鲜缓存、候选上限与降级 | 完成 | `5623eff` |
| M4.6 OpenAI-compatible LLM 解释 | 完成 | `a81f588`，兼容修复 `00bd503` |
| M4.7 人工真实 API Smoke | 完成 | `764226a`，脱敏执行记录 `3eb015b` |
| M4.8 单 Docker 镜像 | 完成 | `cd04abe`，Railway 适配 `ff1a99b`、`6e9b16f` |
| M4.9 GitLab/GitHub CI | 完成 | `6f0d240`、`219437a` |
| M4.10 最终文档与证据 | 完成 | `1ad45cb`、`cc47077`、`0383a28`；最终 CI run `31704124041` success |
